function [opt1, opt2, opt3, stat_dCorMax, stat_dCorInfMax, stat_nndCorInfMax] = helper_rADGTIC(X,Y,nThreshs,nRandomisations,nSearchRandomisations,method)
...
count = 1;
[dCorMapThreshsInf, dCorMapThreshs] = helper_findDCorMapThreshs(X,Y,Xthreshs,Ythreshs,nSearchRandomisations,method);
...
%     disp(['=========' num2str(count)])
    [dCorMapThreshsInf_rnd(:,:,randomisationI), dCorMapThreshs_rnd(:,:,randomisationI)] = helper_findDCorMapThreshs(X(rndPerm,:),Y,Xthreshs,Ythreshs,nSearchRandomisations,method);
    count = count + 1;
%     % plot
...
[opt2_x,opt2_y] = find(dCorMapThreshsInf==stat_dCorInfMax,1);
[opt2_lx, opt2_ux] = find(indexX==opt2_x,1);
[opt2_ly, opt2_uy] = find(indexY==opt2_y,1);
opt2.lx = Xthreshs(opt2_lx);
opt2.ux = Xthreshs(opt2_ux);
opt2.ly = Ythreshs(opt2_ly);
opt2.uy = Ythreshs(opt2_uy);

[opt3_x,opt3_y] = find(cdCorMapThreshsInf_rnd_noiseStdUnits==stat_nndCorInfMax,1);
[opt3_lx, opt3_ux] = find(indexX==opt3_x,1);
[opt3_ly, opt3_uy] = find(indexY==opt3_y,1);
opt3.lx = Xthreshs(opt3_lx);
opt3.ux = Xthreshs(opt3_ux);
opt3.ly = Ythreshs(opt3_ly);
opt3.uy = Ythreshs(opt3_uy);
