/*  
    ...
    b) D. Albanese, M. Filosi, R. Visintainer, S. Riccadonna, G. Jurman,
       ...
       http:sbioinformatics.oxfordjournals.org/content/early/
...
int OptimizeXAxis(double *dx, double *dy, int n, int *Q_map, int q, 
		  int *P_map, int p, int x, double *score, double *mi, int *xbins, int *ybins)
{
  int i, s, t, l;
  int *c;
  int **cumhist;
  ...
  for (i=p+1; i<=x; i++)
    I[p][i] = I[p][p];
  /*****************************************************************/
  /* THIS IS THE FUNCTION THAT NORMALIZES THE MUTUAL INFORMATION
  /* TO GET NON-NORMALIZED MUTUAL INFORMATION, JUST NEED TO STORE VALUE OF THE DENOMONATOR
  /* score */
  for (i=2; i<=x; i++) {
    score[i-2] = I[p][i] / MIN(log(i), log(q));
    mi[i-2] = I[p][i];  /* JBK: Underlying MI estimate */
    xbins[i-2] = i; /* JBK: Have to check this */
    ybins[i-2] = q; /* JBK: Have to check this */
  }
  /***************************************************************/
...
mine_score *init_score(mine_problem *prob, mine_parameter *param)
{
  ...
  /* Matrix to store proto-MIC values */
  ...
  /* JBK make matrix to store mutual information values and bin number values */
  score->I = (double **) malloc (score->n * sizeof(double *));
  score->xbins = (int **) malloc (score->n * sizeof(int *));
  score->ybins = (int **) malloc (score->n * sizeof(int *));
  if ((score->I == NULL) || (score->xbins == NULL) || (score->ybins == NULL))
    goto error_score_I;
   
  for (i=0; i<score->n; i++)  {
      score->I[i] = (double *) malloc ((score->m[i]) * sizeof(double));
      score->xbins[i] = (int *) malloc ((score->m[i]) * sizeof(int));
      score->ybins[i] = (int *) malloc ((score->m[i]) * sizeof(int));
      
      if ((score->I[i] == NULL) || (score->xbins[i] == NULL) || (score->ybins[i] == NULL))  {
        for (j=0; j<i; j++)  {
          free(score->I[j]);
          free(score->xbins[j]);
          free(score->ybins[j]);
          goto error_score_I_i;
        }
      }
  }
  return score;
...
{
  int i, j, k, p, q, ret;
  double *xx, *yy, *xy, *yx, *M_temp, *I_temp;
  int *ix, *iy, *xbins_temp, *ybins_temp;
  ...
  /* JBK */
  I_temp = (double *)malloc ((score->m[0]) * sizeof(double));
  xbins_temp = (int *)malloc ((score->m[0]) * sizeof(int));
  ybins_temp = (int *)malloc ((score->m[0]) * sizeof(int));
  if ((I_temp == NULL) || (xbins_temp == NULL) || (ybins_temp == NULL))
    goto error_I_temp;
  ...
  for (i=0; i<score->n; i++)
	...
	goto error_0;
      ...
      ret = OptimizeXAxis(xx, yx, prob->n, Q_map, q, P_map, p,
			  score->m[i]+1, score->M[i], score->I[i], score->xbins[i], score->ybins[i]); /* JBK */
      if (ret)
	goto error_0;
    }
  ...
  for (i=0; i<score->n; i++)
	...
	goto error_0;
      ...
      ret = OptimizeXAxis(yy, xy, prob->n, Q_map, q, P_map, p,
			  score->m[i]+1, M_temp, I_temp, xbins_temp, ybins_temp); /* I_temp added by JBK */
      if (ret)
	goto error_0;
      
      for (j=0; j<score->m[i]; j++)  {
          /* JBK altered somewhat to allow assignment of both M[j][i] and I[j][i] */
          if (M_temp[j] > score->M[j][i]) {
             score->M[j][i] = M_temp[j];
             score->I[j][i] = I_temp[j];  /* JBK */
             score->xbins[j][i] = xbins_temp[j]; /* JBK */
             score->ybins[j][i] = ybins_temp[j]; /* JBK */
          }
      }
    }
 ...
 error_xx:
  ...
  free(score->M);
  free(score->I); /* JBK */
  free(score->xbins); /* JBK */
  free(score->ybins); /* JBK */
  free(score->m);
  free(score);
 error_score:
  return NULL;
...
/* JBK Returns the Mutual Information correspoinding to MIC. */
double mine_mi(mine_score *score)
{
  int i, j;
  double score_max = 0.0;
  double mi = 0.0;
  ...
  for (i=0; i<score->n; i++) {
    for (j=0; j<score->m[i]; j++) {
      /* If score is greater, change both score_max and mi */
      if (score->M[i][j] > score_max)  {
         score_max = score->M[i][j];
         mi = score->I[i][j];
      }
  ...
  return mi;  /* RETURN UNDERLYING MUTUAL INFORMATION VALUE */
}

/* JBK Returns the number of x-bins used */
int mine_xbins(mine_score *score)
{
  int i, j;
  double score_max = 0.0;
  int xbins = 0;
  ...
  for (i=0; i<score->n; i++) {
    for (j=0; j<score->m[i]; j++) {
      /* If score is greater, change both score_max and xbinss */
      if (score->M[i][j] > score_max)  {
         score_max = score->M[i][j];
         xbins = score->xbins[i][j];
      }
    }
  }
...
/* JBK Returns the number of x-bins used */
int mine_ybins(mine_score *score)
{
  int i, j;
  double score_max = 0.0;
  int ybins = 0;
  ...
  for (i=0; i<score->n; i++) {
    for (j=0; j<score->m[i]; j++) {
      /* If score is greater, change both score_max and ybins */
      if (score->M[i][j] > score_max)  {
         score_max = score->M[i][j];
         ybins = score->ybins[i][j];
      }
    }
  }
  ...
  if (score_ptr != NULL)
      ...
      if (score_ptr->n != 0)
	{
	  ...
	  for (i=0; i<score_ptr->n; i++) {
	    free(score_ptr->M[i]);
	    free(score_ptr->I[i]);
	    free(score_ptr->xbins[i]);
	    free(score_ptr->ybins[i]);
	  }
	  free(score_ptr->M);
	  free(score_ptr->I);
	  free(score_ptr->xbins);
	  free(score_ptr->ybins);
	}
