function p=doubleADICtest(X,Y)

% adaptive distance-based independence criterion (ADIC) test
% a general robust test for mutual information between X and Y
%
% X: n-by-dx matrix of observations for variable X
% Y: n-by-dy matrix of observations for variable Y
% n is the number of observations, which must be i.i.d.
% The observations are listed in the same order in X and Y.
% The dimensionality of X is dx and need not match the dimensionality of Y,
% dy.
%
% The returned p value is the probability that the ADIC is as large as
% observed or larger under the null hypothesis that X and Y are
% independent.
...
snpInf=sharedNeighPairInflation(distX,distY,distXthreshs,distYthreshs);
...
for randomisationI=1:nRandomisations
    ...
    snpInf_rnd(:,:,randomisationI) = sharedNeighPairInflation(squareform(distX_sq_rnd),distY,distXthreshs,distYthreshs);
...
subplot(2,2,4); title(['p = ',num2str(p)]);


function snpInf=sharedNeighPairInflation(distX,distY,distXthreshs,distYthreshs)
